...
- **Blocks are vendored, not authored.** All shadcn blocks (login/signup/dashboard/sidebar — the
  full `__blocks__` catalog, 27 real blocks) live under `src/blocks/<name>/`, generated by
  `scripts/vendor-blocks.mjs`: it fetches each block from the `new-york-v4` registry, rewrites the
  `@/registry/new-york-v4/*` aliases to local relative imports (cn/components/hooks/sibling files),
  and emits `src/blocks/index.ts` (each block's default Page re-exported under a unique PascalCase
  name, surfaced as the `./blocks` export) plus one Storybook story per block. **Edit the script,
  not the generated files** — re-run it to re-vendor. The few strict-TS fixups and per-block axe
  opt-outs (for upstream a11y quirks rendered headlessly) are encoded in the script.
...
src/
  ...
  blocks/       vendored shadcn blocks (full-page compositions), one dir each + index.ts barrel
...
scripts/        gen-styles.ts · vendor-blocks.mjs · copy-styles.mjs
stories/        component example stories + blocks/ (one story per block)
tests/          contrast gate
